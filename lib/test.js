...
function testSend() {
    MyJSON.listen((object, from) => { 
        ...
        if (object.data == 1) {
            ...
            MyJSON.send({address: "localhost", port: 4567}, { packageType: Lib.PACKAGE_TYPES.data, 'data': 3 });
        }
        else if (object.data == 3) {
            console.log("Received three! Stopping...");
            return false;
        }
     }, [Lib.PACKAGE_TYPES.command.value, Lib.PACKAGE_TYPES.data, Lib.PACKAGE_TYPES.hello], [], function () {
        MyJSON.sendBroadcast({ packageType: Lib.PACKAGE_TYPES.hello, 'data': 1 });
        setTimeout(MyJSON.send({address: "localhost", port: 4567}, { packageType: Lib.PACKAGE_TYPES.command.value, 'data': 2 }), 10);
