...
const VALUES = {
	packageType: [ Lib.PACKAGE_TYPES ],
	nodeClass: [ Lib.NODE_CLASSES ],
	nodeCategory: [ Lib.NODE_CATEGORIES ]	
};
...
function validValue(key, value) {
    ...
    if (possibleValues !== undefined) {
        if (Array.isArray(value) && acceptArrays) {
            for (i in value) {
                var val = value[i];
                if (possibleValues.get(val) === undefined || possibleValues.get(val).key.indexOf("|") !== -1)
...
function decode (rawPackage, callback) {
    cbor.decodeFirst(rawPackage, (error, package) => {
        callback(error, error === null ? exchangeKeys(package, (key) => FIELDS.get(Number(key)).key,
            (key, value) => validValue(FIELDS.get(Number(key)).key, value)) : null);
    });
}
...
Lib.prototype.listen = function (objectCallback, packageTypes, addresses, listenCallback) {
    ...
    if (packageTypes != null)
        validValue(FIELDS.packageType, packageTypes)
