...
Lib.prototype.encode = function(json) {
	var new_json = {};
	...
	return cbor.encode(new_json);
}

Lib.prototype.decode = function(enc_obj, cb){
    var decodedObj = {};
    cbor.decodeFirst(enc_obj, (err, obj) => {
        if(err) cb(err, null);
        else{
            for(key in obj){
                key = parseInt(key);
                ...
                //console.log(key);
                if (field_def_values !== undefined) {
                    var value_dec = field_def_values.get(obj[key]).key;
                    if(value_dec === undefined)
                        throw("Error: value " + [key] + " is undefined");
                    decodedObj[key_dec] = value_dec;
                }
                else decodedObj[key_dec] = obj[key];
            }
            cb(null, decodedObj);
        }
    });
